...
\emph{cummeRbund 2.0 was designed in conjunction with the release of cufflinks 2.0.  While we attempted to preserve backwards-compatability, it is highly recommended
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_dens}
}
\qquad
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_dens_rep}}
\end{center}
\end{figure}
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_box}
}
\qquad
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_box_rep}}
\end{center}
\end{figure}
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_dendro}
}
\qquad
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-global_plots_dendro_rep}}
\end{center}
\end{figure}
...
<<label=global_plots_MA,fig=TRUE,echo=FALSE>>=
<<global_plots_4>>
print(m)
print(mCount)
@
...
Vectors of sample namesand feature names are available by using the \Rmethod{samples} and \Rmethod{featureNames} methods:
...
<<label=geneset_plots_barplot,fig=TRUE,echo=FALSE>>=
<<geneset_plots_1.5>>
print(b)
@
...
<<label=geneset_plots_dendro,fig=TRUE,echo=FALSE>>=
<<geneset_plots_5>>
plot(den)
@
...
An individual CuffGene object can be created by using the \Rfunction{getGene} function for a given 'gene\_id' or 'gene\_short\_name'.
<<gene_level_1>>=
myGeneId<-"PINK1"
...
gl
@
<<gene_plots_replicate_1,include=FALSE>>=
...
\includegraphics[width=0.45\textwidth]{cummeRbund-manual-gene_plots_line}
}
\qquad
...
<<gene_plots_replicate_1>>
print(gl.rep)
@

\includegraphics[width=0.45\textwidth]{cummeRbund-manual-gene_plots_replicate_line}}
\end{center}
\end{figure}
...
icp
@
As of v1.1.1 of \Rpackage{cummeRbund}, the output of csCluster is a modified \Rclass{pam} object. This replaces the default plotting behavior of the original csCluster plot to allow for further analysis of the clustering results. The original plotting behavior has been recapitulated
