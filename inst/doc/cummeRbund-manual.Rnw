...
An individual CuffGene object can be created by using the \Rfunction{getGene()} function for a given 'gene\_id' or 'gene\_short\_name'. As of cummeRbund $\gev2.0$ you can also use isoform\_id, tss\_group\_id, or
...
\begin{figure}[htp]
	\begin{center}
	\subfloat[K-medioids clustering with Jensen-Shannon distance for a CuffGeneSet.]{
